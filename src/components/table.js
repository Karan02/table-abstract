...
class Table extends React.Component {
  ...
  AddEntries = addContent => {
    ...
    for (var i = 0; i < this.props.columns.length; i++) {
      newObject2[this.props.columns[i].title] =
        addContent[this.props.columns[i].title];
    }
  ...
  handleAdd = () => {
    ...
    inputs[`${this.props.columns.length}`] = (
  ...
  arrangeText = (key, arrangeMethod) => {
    ...
    if (arrangeMethod === "asc") {
      arraycopy.sort(function(a, b) {
        if (a[Object.keys(a)[key]] < b[Object.keys(b)[key]]) {
          return -1;
        }
        if (a[Object.keys(a)[key]] > b[Object.keys(b)[key]]) {
          return 1;
  ...
  render() {
    ...
    for (let i = 0; i < this.state.arrayOk.length; i++) {
      if (this.state.arrayOk[i] && this.state.arrayOk[i] !== "") {
        handleArray = handleArray.filter(item => {
          return (
            item[Object.keys(item)[i]].indexOf(this.state.arrayOk[i]) !== -1
          );
        });
      }
    }
    ...
    let intermediateArray3 = [];
    let intermediateArray3bool = false;
    this.state.arrayFilter.forEach((element, index) => {
      if (element) {
        element.forEach((each, indexinner) => {
          if (each) {
            if (each.isActive) {
              console.log(pushArray, "push array");
              intermediateArray = pushArray.filter(row => {
                return row[Object.keys(row)[index]].indexOf(each.text) !== -1;
              });
              intermediateArray2 = intermediateArray2.concat(intermediateArray);
              console.log("intermediate array", intermediateArray);
            }
          }
        });
        intermediateArray3bool = true;
        // continue your work here
        if (intermediateArray2.length) {
          console.log("1");
          pushArray = intermediateArray2;
          intermediateArray3 = intermediateArray2;
          intermediateArray2 = [];
        }
      }
    });
      ...
      <div>
        <div className={this.state.blur ? "blur" : null}>
          <table>
            ...
            {intermediateArray3.length && intermediateArray3bool ? (
              <Content
                handleDelete={this.handleDelete}
                columns={this.props.columns}
                data={intermediateArray3}
          ...
          <div className="addInput">
            <button className="addButton" onClick={this.showAddSection}>
              Add
            </button>
            <br />
            <div className="inputs">{this.handleAddSection()}</div>
          </div>
        </div>
      </div>
    );
